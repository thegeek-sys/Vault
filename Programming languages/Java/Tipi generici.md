...
>         // utilizzo le Collection poiché i tipi delle Collection sono generici (non posso mettere un tipo generico in un array)        Mela[] mele = new Mela[] {new Mela(), new Mela()};  
...
>         // non ho errori sull'ide perché l'errore di consistenza sul tipo vengono fatti solamente dal compilatore        // il compilatore mi restituisce ArrayStoreException        copy(mele, pere);  
>   
>           
...
>   
>         // provo quindi a creare un arraylist di frutti ma anche in questo caso mi viene dato erroreList<Frutto> lfrutti = Arrays.asList(new Frutto[]{new Pera(), new Mela()});  
