...
## Memory hierachy
### Memory access
 Data allocated on host memory is not visible form the GPU, and viceversa
 
```c
int *mydata = new int[N]; malloc(sizeof(int), ) ....
// populating the array
foo<<<grid, block>>>(mydata, N) // not possible
```

Instead, it must explicetely be copied from/to host to GPU

### Memory Allocation and Copy
chiamate che vengono fatte sull’host