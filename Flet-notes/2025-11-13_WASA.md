...
## Output: $\verb|http.ResponseWriter|$
Il parametro `w` (di tipo `http.ResponseWriter`) è l’interfaccia usata per costruire e inviare la risposta al client.

### Impostazione degli header
Gli header devono essere impostati prima di scrivere il corpo della risposta

| Funzione                        | Descrizione                                                                 | Utilizzo Tipico                                  |
|---------------------------------|-----------------------------------------------------------------------------|-------------------------------------------------|
| `w.Header().Set("Name", "Value")` | Imposta un header.                                                          | `w.Header().Set("Content-Type", "application/json")` |
| `w.WriteHeader(code)`            | Imposta lo status code HTTP. Se non chiamata, Go usa di default 200 OK.    | `w.WriteHeader(http.StatusCreated)` (201)       |

### Scrittura del corpo della risposta
Dopo aver impostato gli header, si scrive il corpo

| Funzione                     | Descrizione                                                | Utilizzo Tipico                             |
|-------------------------------|------------------------------------------------------------|--------------------------------------------|
| `w.Write([]byte)`             | Scrive un array di byte sul corpo.                         | Usato per dati grezzi o JSON codificato.   |
| `fmt.Fprintf(w, format, args...)` | Funzione comoda che scrive direttamente su un `io.Writer` (come `w`). | Usato spesso per risposte semplici in testo semplice. |
